...
Triangle::Triangle(Point p, Point q, Point r)	//counter clockwise
{
    a = p;
...
Point Triangle::inCenter()				//intersection of the angle bisectors, center of inscribed circle
{
    double p = perm();
...
Point Triangle::circumCircle() 
{
    ...
    // THIS SHOULD BE IN YOUR MAIN
    // if (abs(g) < EPS) return NULL;

    return Point((cay*e - bay*f) / g, (bax*f - cax*e) / g);

}

double Triangle::areaMedians(double ma, double mb, double mc)		//medians of the triangle
{
    double s = (ma + mb + mc) / 2.0;							//max(ma, mb, mc) < s
    return sqrt(s * (s - ma) * (s - mb) * (s - mc)) * 4.0 / 3.0;
}

double Triangle::areaHeights(double ha, double hb, double hc)		//heights of the triangle
{
